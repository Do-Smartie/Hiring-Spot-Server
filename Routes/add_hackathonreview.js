...
const hackathonreviewadder = express.Router()

hackathonreviewadder.post('/',async(req,res)=>{
    try{
        const {nameOfTheRound,roundNo,companyName,batch,role,Package,problemStatements,level,hackathonDuration,feedBack,status}  = req.body
        ...
        if(roundno===1){
            if(mcqreviewAvailable.length!==0 || gdreviewAvailable.length!==0 || hackathonreviewAvailable!==0 || techhrreviewAvailable.length!==0 || personalreviewAvailable.length!==0 || codingreviewAvailable.length!==0){
                ...
                const addhackathonreview = new HackathonReview({
                    username : username,
                    rollNo : rollNo,
                    ...
                    level : level,
                    hackathonDuration : hackathonDuration,
                    feedBack : feedBack,
                    status : status
                })
                await addhackathonreview.save()
                return res.status(200).json({
                    Success : true,
                    Message : "Hackathon Round Review Added Successfully!"
                })
            }
        }else if(roundno>1){
        ...
        return res.status(404).json({
            Success : false,
            Message : "There is an Error in Adding Your Hackathon Round Review! Please Try Again After Sometimes..."
        })
    }
})


module.exports = hackathonreviewadder