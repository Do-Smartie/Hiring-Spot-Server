...
const mcqReviewAdder = express.Router()

mcqReviewAdder.post('/',async(req,res)=>{
    try{
        ...
        if(roundno===1){
            if(mcqreviewAvailable.length!==0 || gdreviewAvailable.length!==0 || hackathonreviewAvailable!==0 || techhrreviewAvailable.length!==0 || personalreviewAvailable.length!==0 || codingreviewAvailable.length!==0){
                ...
                const addmcqreview = new McqReview({
                    username : username,
                    rollNo : rollNo,
        ...
        return res.status(404).json({
            Success : false,
            Message : "There is an Error in Adding Your MCQ Review! Please Try Again After Sometimes..."
        })
    }
})

module.exports = mcqReviewAdder