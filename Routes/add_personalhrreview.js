...
const personalhrreviewAdder = express.Router()


personalhrreviewAdder.post('/',async(req,res)=>{
    try{
        const {nameOfTheRound,roundNo,companyName,batch,role,Package,typeOfQuestions,interviewDuration,feedBack,status}  = req.body
        ...
        if(roundno===1){
            if(mcqreviewAvailable.length!==0 || gdreviewAvailable.length!==0 || hackathonreviewAvailable!==0 || techhrreviewAvailable.length!==0 || personalreviewAvailable.length!==0 || codingreviewAvailable.length!==0){
                ...
                const addpersonalhrreview = new PersonalHrReview({
                    username : username,
                    rollNo : rollNo,
                ...
                await addpersonalhrreview.save()
                return res.status(200).json({
                    Success : true,
                    Message : "Personal HR Round Review Added Successfully!"
                })
            }
        }else if(roundno>1){
        ...
        return res.status(404).json({
            Success : false,
            Message : "There is an Error in Adding Your Personal HR Round Review! Please Try Again After Sometimes..."
        })
    }
})


module.exports = personalhrreviewAdder
